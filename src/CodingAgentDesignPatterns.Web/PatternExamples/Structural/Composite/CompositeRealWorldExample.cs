...
public static class CompositeRealWorldExample
{
    ...
    {
        var root = new Group("Photos");
        root.Add(new Leaf("Beach.jpg", 8));
        root.Add(new Leaf("Mountains.jpg", 12));

        return new[]
        {
            "Folder tree",
            $"Root: {root.Name}",
            $"Total MB: {root.GetTotal()}"
        };
    }
